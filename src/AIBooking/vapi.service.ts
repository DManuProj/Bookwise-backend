...
//     const slug = params.slug?.trim();
//     if (!slug) {
//       return { error: 'Missing slug parameter. Always pass slug in tool calls.' };
//     }
//     const org = await this.prisma.db.organisation.findUnique({
//       where: { slug },
//       include: {
//         services: {
//           where: { isActive: true },
//           select: { id: true, name: true, durationMins: true, price: true, description: true },
//         },
//       },
//     });
//     if (!org || org.isDeleted) return { error: 'Business not found' };
...
//     if (!slug) return { error: 'Missing slug parameter. Always pass slug in tool calls.' };
//     if (!date) return { error: 'Missing date parameter. Ask the customer which day they want.' };
//     if (!serviceId) {
//       return { error: 'Service not found or not selected yet. Ask the customer which service they want to book BEFORE checking time slots.' };
//     }
...
//     if (!slug) return { error: 'Missing slug parameter. Always pass slug in tool calls.' };
//     if (!date) return { error: 'Missing date parameter. Ask the customer which day they want.' };
//     if (!time) return { error: 'Missing time parameter. Ask the customer what time they want.' };
//     if (!serviceId) {
//       return { error: 'Service not found or not selected yet. Ask the customer which service they want to book first.' };
//     }
//     if (!customerName) {
//       return { error: 'Missing customer name. Please ask the customer for their name before booking.' };
//     }
//     if (!customerEmail) {
//       return { error: 'Missing customer email. Please ask the customer for their email before booking.' };
//     }
//     if (!customerPhone) {
//       return { error: 'Missing customer phone. Please ask the customer for their phone number before booking.' };
//     }
//     if (!customerEmail.includes('@')) {
//       return { error: 'Customer email looks invalid. Please confirm the email with the customer.' };
//     }
...
//     if (activeStaffCount === 0) {
//       return {
//         error: 'This business does not have any active staff to take bookings right now. Please contact them directly.',
//       };
//     }
...
//     if (requestedStaffId && !activeStaffIds.includes(requestedStaffId)) {
//       return {
//         error: 'The requested staff member is no longer available. Please ask the customer to pick again or try another time.',
//       };
//     }
...
//       if (bookingsThisMonth >= cap) {
//         return {
//           error: 'This business is not accepting online bookings at the moment. Please contact them directly to schedule.',
//         };
//       }
//     }
...
//         if (requestedStaffId) {
//           // Specific staff requested — verify they're still free
//           const isBusy = overlappingBookings.some((b) => b.userId === requestedStaffId);
//           if (isBusy) throw new Error('REQUESTED_STAFF_TAKEN');
//           finalStaffId = requestedStaffId;
//         } else {
//           // No preference — check overall capacity
...
//       if (msg === 'SLOT_TAKEN') {
//         return {
//           error: 'This time slot is no longer available — all staff are booked then. Please pick another time.',
//         };
//       }
//       if (msg === 'REQUESTED_STAFF_TAKEN') {
//         return {
//           error: 'The requested staff member just got booked at that time. Please offer the customer another time or a different staff member.',
//         };
//       }
//       throw err;
//     }
...
//     if (!callId) {
//       this.logger.warn('end-of-call-report missing call.id — cannot track usage');
//       return;
//     }
//     if (!slug) {
//       this.logger.warn(`Call ${callId} missing slug — cannot attribute usage`);
...
//     } catch (err) {
//       if ((err as any).code === 'P2002') {
//         this.logger.log(`Duplicate webhook for callId ${callId} — already tracked`);
//       } else {
//         this.logger.error(`Failed to track voice usage: ${(err as Error).message}`);
//       }
//     }
...
//         this.logger.log(`Transcript saved for booking: ${booking.id}`);
//       } else {
//         this.logger.log(`No booking for call ${callId} — minutes tracked, no transcript link`);
//       }
//     }
//   }
// }
